...
void gru_forward_pass(
    ...
    for (int seq = 0; seq < num_sequences; ++seq) {
        for (int t = 0; t < time_frames ; t++) {
            ...
            float *x_t = inputs + seq * time_frames * input_size + t * input_size;

            // Compute z_t = sigmoid(W_z * x_t + U_z * h_prev + b_z)
            matrix_vector_multiply(W_z, x_t, z, hidden_size, input_size);                           //W_z * x_t
            matrix_vector_multiply(U_z, h_prev + seq * hidden_size, z, hidden_size, hidden_size);  //U_z * h_prev
            ...
            matrix_vector_multiply(U_r, h_prev + seq * hidden_size, r, hidden_size, hidden_size);   //U_r * h_prev
            for (int i = 0; i < hidden_size; ++i) {
            ...
            for (int i = 0; i < hidden_size; ++i) {
                r_h_prev[i] = r[i] * h_prev[seq * hidden_size + i];
            }
            ...
            for (int i = 0; i < hidden_size; ++i) {
                h[i] = ( z[i]) * h_prev[seq * hidden_size + i] + (1.0 -z[i]) * h_hat[i];
            }


            // Store the output hidden state for this time step
            for (int i = 0; i < hidden_size; ++i) {
                output[seq * time_frames * hidden_size + t * hidden_size + i] = h[i];
            }
...
int main() {
    ...
    float inputs[num_sequences * time_frames * input_size]; // inputs
    float h_prev[num_sequences * hidden_size]; // hidden state
    ...
    float output[num_sequences * time_frames * hidden_size];
    ...
    load_weights("D:\\new job\\IPHIPI\\txt\\inputs.txt",inputs, num_sequences * time_frames * input_size);

    //initializing the hidden units for 1st time as all zeros
     for (int i = 0; i < num_sequences * hidden_size; ++i) {
        h_prev[i] = 0;
    }
    ...
    gru_forward_pass(num_sequences, time_frames, input_size, hidden_size,inputs, h_prev,
                     W_z, U_z, b_z,
                     W_r, U_r, b_r,
                     W_h, U_h, b_h,
                     output);
    ...
    for (int i = 0; i < num_sequences * time_frames * hidden_size; ++i) {
        printf("%f ", output[i]);
